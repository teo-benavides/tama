...
_TamaScriptRepository::_make_resolver(const std::string &scripts_dir,
    ...
    return [this, scripts_dir, loading](const std::string &name) mutable -> std::shared_ptr<_TamaASTNode> {
        for (const auto &n : loading) {
            if (n == name) {
                UtilityFunctions::push_warning(
                    String("_TamaScriptRepository: circular include '") + String(name.c_str()) + "'");
                return std::shared_ptr<_TamaASTNode>();
            }
        }
        ...
        if (fsrc.is_empty()) return std::shared_ptr<_TamaASTNode>();
        loading.push_back(name);
        std::string src = fsrc.utf8().get_data();
        std::shared_ptr<_TamaASTNode> prog = _parse_source(src, path, _make_resolver(scripts_dir, loading));
...
std::shared_ptr<_TamaASTNode> _TamaScriptRepository::_parse_source(const std::string &source,
    ...
    while (!file.is_empty()) {
        if (file.ends_with(".tam") || file.ends_with(".tama")) {
            ...
            if (!src.is_empty()) {
                ...
                std::shared_ptr<_TamaASTNode> prog = _parse_source(src.utf8().get_data(), full.utf8().get_data(), res);
                if (prog)
...
void _TamaScriptRepository::load_script(const String &filename, const String &full_path) {
    ...
    Resolver res = _make_resolver(_scripts_dir);
    std::shared_ptr<_TamaASTNode> prog = _parse_source(src.utf8().get_data(), full_path.utf8().get_data(), res);
    if (prog)
...
void _TamaScriptRepository::load_script_from_source(const String &name, const String &source) {
    Resolver res = _make_resolver(_scripts_dir);
    std::shared_ptr<_TamaASTNode> prog = _parse_source(source.utf8().get_data(), name.utf8().get_data(), res);
    if (prog)
        _scripts[name.utf8().get_data()] = prog;
}

_TamaASTNode *_TamaScriptRepository::get_tama_script(const String &filename) const {
    auto it = _scripts.find(filename.utf8().get_data());
    if (it != _scripts.end()) return it->second.get();
    return nullptr;
}
...
_TamaASTNode *_TamaScriptRepository::parse_source_for_exports(const String &source,
                                                               const String &scripts_dir) {
    Resolver res = _make_resolver(scripts_dir.utf8().get_data());
    std::shared_ptr<_TamaASTNode> prog = _parse_source(source.utf8().get_data(), "<editor>", res);
